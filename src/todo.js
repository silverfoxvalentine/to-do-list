...
export const displayTodo = function(todo, appendDataTo) {
    ...
    editBtn.addEventListener('click', (e)  => { 
        if (!inputOpen) {
            ...
            displayTodoDataInput(`${todo.name}`, `${todo.name}`, `${todo.description}`,`${todo.description}`, `${todo.date}`, `${todo.priority}`, `${todo.project}`, todoCard, confirmEditTodo, discardEditTodo)
            todoCard.firstChild.focus()
            todoCard.addEventListener("keyup", (e) => keyHandler(e))
        }
    })

    const confirmEditTodo = function(e, nameInput, descriptionInput, priorityChoice, projectChoice, dateInput) {    
        const editedTodo = new Todo(nameInput.value, descriptionInput.value, dateInput.value, priorityChoice.value, projectChoice.value, todo.completed)
        const todoCard = e.target.parentElement.parentElement
        ...
        if (editedTodo.project == currentProjectId && !currentCategory ||
            currentCategory === 0 || currentCategory === 1 && isToday(editedTodo.date) || 
...
addTodo.addEventListener('click', () => {
    if (!inputOpen){
        ...
        todoCard.classList.add('todo-input')
        displayTodoDataInput('', 'New To-do', '', 'Description', '', 'Medium', currentProjectId, todoCard, confirmNewTodo, discardNewTodo)
...
const displayTodoDataInput = function(name, namePlaceholder, description, descriptionPlaceholder, date, priority, project, appendInputTo, confirmBtnFunction, discardBtnFunction) {
    ...
    confirmBtn.addEventListener('click', (e) => confirmBtnFunction(e, nameInput, descriptionInput, priorityChoice, projectChoice, dateInput)) 
    discardBtn.addEventListener('click', (e) => discardBtnFunction(e))
}

const confirmNewTodo = function(e, nameInput, descriptionInput, priorityChoice, projectChoice, dateInput ) {   
    let newTodo = new Todo(`${nameInput.value}`, `${descriptionInput.value}`, `${dateInput.value}`, `${priorityChoice.value}`, `${projectChoice.value}`, false)     
    projects[projectChoice.value].todos.push(newTodo)
    e.target.parentElement.parentElement.remove() 
    inputOpen = false  
    
    if ((newTodo.project == currentProjectId && !currentCategory) ||
        currentCategory === 0 || currentCategory === 1 && isToday(newTodo.date) || 
